...
<!-- Data Browser config embedded by leak_predict.bob: ONE consolidated live
     trend carrying all four raw sensors that feed the leak-detection model —
     TT-102 (return temp), TT-110 (supply temp), LI-103 (vessel level) and
     P-101 Speed (pump speed). Replaces the four separate per-sensor .plt
     files (leak_predict_tt102/tt110/li103/p101speed.plt), which produced a
     cramped 2x2 grid of 215x180 plots. Three value axes keep the very
     different units (degC / mm / %) from flattening each other:
       axis 0 (left)  — Temperature, degC  — TT-102, TT-110
       axis 1 (right) — Level, mm          — LI-103
       axis 2 (right) — Pump speed, %      — P-101 Speed
     Trace colors are carried over unchanged from the old per-sensor plots
     so operators keep the same color-to-sensor mapping. -->
<databrowser>
  <title>Live Skid Sensors — TT-102 / TT-110 / LI-103 / P-101 Speed</title>
